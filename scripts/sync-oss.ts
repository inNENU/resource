import { getLastChangedFiles, removeOSSFiles, uploadOSSFiles } from "innenu-generator";
...
const syncOSS = async (): Promise<void> => {
  ...
  const newFiles = [
    ...
    ...getUpdatedResourceNames([...added, ...modified, ...deleted]).map((file) => ({
      local: `.oss/${file}.zip`,
      online: `${file}.zip`,
    })),
  ];
  ...
  await Promise.all([
    ...
    removeOSSFiles(deleted.filter((item) => item.startsWith("img/") || item.startsWith("file/"))),
  ]);
};
