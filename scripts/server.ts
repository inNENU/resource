...
// 代理转发函数
// 使用 fetch 实现代理转发
const proxyToService = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
  ...
  try {
    ...
    if (req.method !== "GET" && req.method !== "HEAD") {
      ...
      req.on("end", () => {
        ...
        sendProxyRequest(targetUrl, fetchOptions, res).catch((err: unknown) => {
...
const handleSettingsRequest = (req: IncomingMessage, res: ServerResponse): void => {
  // 只处理 POST 请求
  if (req.method !== "POST") {
    res.statusCode = 405;
  ...
  req.on("end", () => {
    try {
      ...
      if (!appId || !version || typeof appId !== "string" || typeof version !== "string") {
        res.statusCode = 400;
      ...
      console.error("解析 settings 请求失败:", err);
      res.statusCode = 400;
...
const server = createServer((req, res) => {
  ...
  if (pathname.startsWith("/service/")) {
    // oxlint-disable-next-line promise/prefer-await-to-callbacks
    proxyToService(req, res).catch((err: unknown) => {
      console.error("代理请求失败:", err);
      res.statusCode = 500;
      res.end("代理请求失败");
    });
